...
public abstract class Package {
	...
	public void addTracking(Tracking t) {
		tracking.add(t);
		Write("Package ID: " + this.packageID + " Address sender: " + this.getSenderAddress() + " Status: " + t.toString());
	}
	...
	public void Write(String s) {
		MainOffice.getInstance().getLock().writeLock().lock();
		try {
			FileWriter myWriter = new FileWriter(MainOffice.getInstance().getFile(), true);
			myWriter.write(s + "\n");
			myWriter.close();
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
			MainOffice.getInstance().getLock().writeLock().unlock();
		}
	}
}
